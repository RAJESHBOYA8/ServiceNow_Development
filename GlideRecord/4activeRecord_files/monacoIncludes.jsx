...
helpMacro: getMessage("/*The Syntax Editor macros are:\n-----------------------------"),
		...
		this.accessKey = this.options.accessKey.replace("CTRL", "Ctrl").replace("SHIFT", "Shift").replace("OPT", "Alt").replace("ALT", "Alt").replace(" + ", "-");
...
return true;
			...
			el.on("mousedown", (function(event) {
				...
				if ("Ctrl".indexOf(this.options.accessKey) === 0)
		...
		this.editor.onDidFocusEditorText((() => {
			...
			this.editor.addCommand(monaco.KeyMod.WinCtrl | monaco.KeyCode.KeyM, (() => {
	...
	getExtraKeyMap: function() {
		...
		return {
			"showKeyMap": accessKey + "-H",
			"fullScreen": "Ctrl-M",
			"find": accessKey + "-F",
			"findNext": "Enter",
			"findPrev": "Shift-Enter",
			"formatCode": "Shift-Alt-F",
			"replace": accessKey + "-Alt-F",
			"replaceAll": accessKey + "-Alt-F",
"commentSelection": accessKey + "-/"
		}
	},
	toString: function () {
...
var ie = /MSIE \d/.test(navigator.userAgent);
	...
	replace: function(editor) {
		...
		$$("div.codicon-replace")[0].style.display = "";
		$$("div.codicon-replace-all")[0].style.display = "none";
	},
	replaceAll: function(editor) {
		editor.getAction("editor.action.startFindReplaceAction").run();
		$$("div.codicon-replace-all")[0].style.display = "";
		$$("div.codicon-replace")[0].style.display = "none";
	},
...
this.editorCacheWorker = new SharedWorker("scripts/classes/monaco/editorCacheWorker.js?v=06-04-2025_0434&sysparm_substitute=false");
...
var autoCompleteWorker = new SharedWorker("scripts/classes/monaco/autoCompleteWorker.js?v=06-04-2025_0434&sysparm_substitute=false");
...
var discoverWorker = new SharedWorker("scripts/classes/monaco/discoverWorker.js?v=06-04-2025_0434&sysparm_substitute=false");
...
var linterUrl = "/scripts/snc-code-editor/eslint_bundle.min.js?v=06-04-2025_0434&sysparm_substitute=false";
var scriptUrl = "scripts/classes/monaco/lintWorker.js?v=06-04-2025_0434&sysparm_substitute=false";
...
helpList.push("*/");
	...
	formatCode: function() {
		...
		if (language === this.languages.JAVASCRIPT || language === this.languages.HTML) {
			...
			this.editor.executeEdits("formatCode", [
				{
					range: selection,
					text: content
				}
...
var apiDocUrl = developerDocUrl + "/go_to_api.do?v=" + getVersion() + "&resource_id=CSHelp:";
...
var top = topForLineNumber - editorScrollTop - (nodeHeight - lineHeight) / 2;
		...
		var widgetTop = Math.max(Math.round(top + 4), 4) + "px";
